...
def modelMembraneReturnSpikeTuple(t0, n, dt, Iext, tm1):
    if(t0 + n*dt == 0):
...
def getSynapticCurrentListedRates(r1s, r2s, iterations, dt, Ts):
    currentQuantumns = []
    synapseStates = np.zeros((100))
    # run for one second
    for i in range(iterations):
...
if(q3 == True):
    ...
    quantizedInputCurrents_1 = getSynapticCurrentListedRates(r1_rates_1, r2_rates_1, iterations, dt, Ts)
    quantizedInputCurrents_2 = getSynapticCurrentListedRates(r1_rates_2, r2_rates_2, iterations, dt, Ts)
    quantizedInputCurrents_3 = getSynapticCurrentListedRates(r1_rates_3, r2_rates_3, iterations, dt, Ts)
    # now that we have a function of current, let's run the synapse model
    vms_1 = []
    timeSteps = []
    lastVmValues = []
    ...
    for i in (list(range(iterations))):
        vm, spike1 = modelMembraneReturnSpikeTuple(0, i, dt, quantizedInputCurrents_1[i],tm)
        vms_1.append(vm)
        spikes1 += spike1
        timeSteps.append(i * dt)
    ...
    spikes2 = 0
    for i in (list(range(iterations))):
        vm, spike2 = modelMembraneReturnSpikeTuple(0, i, dt, quantizedInputCurrents_2[i],tm)
        spikes2 += spike2
        vms_2.append(vm)
    ...
    spikes3 = 0
    for i in (list(range(iterations))):
        vm, spike3 = modelMembraneReturnSpikeTuple(0, i, dt, quantizedInputCurrents_3[i],tm)
        spikes3 += spike3
        vms_3.append(vm)
   ...
   # plt.plot(timeSteps, vms_3, label = "B1 = 0, B2 = 5")
    ...
    print("B1 = 0, B2 = 5 spikes: ", spikes2)
    plt.legend(loc='lower right')
    plt.title("LIF Model Potential as a function of time with sinosoidal synaptic fire rates")
    plt.xlabel('Time Elapsed / (s)', fontsize=16)
    plt.ylabel('Neuron Potential / (V)', fontsize=14)
    plt.show()
